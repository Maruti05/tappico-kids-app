          ...
          SafeArea(
            ...
            child: Column(
              children: [
                ...
                const InfoHeader(label: 'Learn your birds! 🦉'),
                Expanded(
                      ...
                      return RepaintBoundary(
                        child: TappableCard(
                          ...
                          onTap: (position) async {
                            ...
                            ref.read(_tappedBirdProvider.notifier).set(item);
                            ref.read(ttsServiceProvider).speak(item.ttsPhrase);
                            await Future.delayed(const Duration(milliseconds: 2200));
                            if (ref.read(_tappedBirdProvider)?.name == item.name) {
          ...
          if (tapped != null)
            TapOverlay(
              color: AppColors.letterColors[
                birdData.indexWhere((e) => e.name == tapped.name) %
                AppColors.letterColors.length
              ],
              onDismiss: () => ref.read(_tappedBirdProvider.notifier).set(null),
              child: Column(
